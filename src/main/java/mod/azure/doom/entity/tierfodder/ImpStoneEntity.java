...
public class ImpStoneEntity extends DemonEntity implements IAnimatable {
	...
	public ImpStoneEntity(EntityType<ImpStoneEntity> entityType, World worldIn) {
	...
	private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
		if (event.isMoving()) {
			event.getController().setAnimation(new AnimationBuilder().addAnimation("spinning", true));
	...
	public void tickMovement() {
		if (this.world.isClient && (this.isAttacking() || !(lastLimbDistance > -0.15F && lastLimbDistance < 0.15F))) {
			this.world.addParticle(ParticleTypes.SOUL_FIRE_FLAME, this.getParticleX(0.2D), this.getRandomBodyY(),
					this.getParticleZ(0.5D), 0.0D, 0D, 0D);
		}
		super.tickMovement();
	}

	public static boolean spawning(EntityType<ImpStoneEntity> p_223337_0_, World p_223337_1_, SpawnReason reason,
